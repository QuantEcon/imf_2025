...
## Local installs
  ...
  ### Alternate installation method: `uv`

    You can install `uv` for your operating system by following the instructions
    in the [documentation](https://docs.astral.sh/uv/getting-started/installation/).

    Once `uv` is installed, you can download all of the relevant packages by
    entering this workshop's directory in the terminal or Windows powershell and
    typing `uv sync`.

    To start a Jupyter notebook, run `uv run jupyter lab` from the same terminal
