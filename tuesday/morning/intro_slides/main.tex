...
\begin{frame}{Training LLMs 1.0}
    ...
    \begin{enumerate}
        \item Learning machine reads in $n$ tokens (input, context window)
        \item Machine assigns pmf over next token (output)
        \item Prediction compared to observed token using KL distance
        \item Loss recorded and window shifted forward one token
    \end{enumerate}
    ...
    \begin{enumerate}
        \item Accumulate losses over the batch to build loss function
        \item Evaluate gradient of loss function at current parameters
...
\begin{frame}{Vending Bench}

    Agents must autonomously run a vending machine business for a year.

    \medskip
    \begin{itemize}
        \item Suppliers may and actively try to exploit the agent 
        \medskip
...
\begin{frame}
    ...
    \begin{figure}
       \begin{center}
        \scalebox{0.36}{\includegraphics{vending.png}}
       \end{center}
    \end{figure}
...
\begin{frame}
    ...
    \medskip
    \brown{No coding skills necessary.}''
...
\begin{frame}
    ...
    Research teams are pushing hard to build ever-more autonomous coding agents.
...
\begin{frame}{Summary}
    ...
    \begin{itemize}
        ...
        \item Better understanding of core tools (hardware / software)
            \vspace{0.2em}
        \item Apply knowledge to economic modeling 
            \vspace{0.2em}
