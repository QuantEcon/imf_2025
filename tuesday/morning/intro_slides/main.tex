...
\begin{frame}{Training LLMs 1.0}
    ...
    \medskip
    Fix offset $n \in \NN$, set $i=1$, initialize ANN parameters $\theta$

    \boldteal{Inner loop}
    %
    \begin{enumerate}
        \item Read in tokens $i, i+1, \ldots, i+n$ (input)
        \item Assign $p = (p_1, \ldots, p_m)$ as prediction of next token (output)
